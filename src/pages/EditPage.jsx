...
export default function EditPage() {
  ...
  // console.log(rooms);
  const { editRepair } = useAuth();
  console.log(editRepair);
  const fileEl = useRef();
  const navigate = useNavigate();
  const [topic, setTopic] = useState(editRepair.title);
  const [details, setDetails] = useState(editRepair.repairDeteil);
  const [roomAt, setRoomAt] = useState(editRepair.roomId);
  const [file, setFile] = useState(null);
  const [phoneNumber, setPhoneNumber] = useState(editRepair.phone);
  const [loading, setLoading] = useState();

  const hdlEditRepair = async () => {
    try {
      ...
      formData.append("id", editRepair.id);
      setLoading(true);
      console.log("formData", formData.getAll("roomAt"));
      await Axios.patch("/request-repair/edit", formData);
